 ...
 * 
 * Design: Industrial IoT / CloudTek Professional Edition
...
// ===== SaaS Design Tokens (UIdesign.md + HTML v9.5 sync) =====
const C = {
...
const S = StyleSheet.create({
  ...
  kpiBadge: { position:'absolute', top:0, right:0, width:32, height:32, borderBottomLeftRadius:12, borderTopRightRadius:16, opacity:0.25 },
  kpiTrendBadge: { marginTop:4, paddingHorizontal:8, paddingVertical:2, borderRadius:99 },
  ...
  navItemA: { },
  navActivePip: { position:'absolute', top:2, left:'25%', right:'25%', height:3, backgroundColor:C.primary, borderRadius:2 },
...
export default function App() {
  ...
  const [consoleMode, setConsoleMode] = useState<'ASCII'|'HEX'>('ASCII');
  ...
  const KPICard = ({ icon, value, label, trend, color, bg }: any) => (
    <View style={[S.kpiCard, { backgroundColor: bg || C.card }]}>
      {bg && <View style={[S.kpiBadge, { backgroundColor: bg }]} />}
      <Text style={S.kpiIcon}>{icon}</Text>
      <Text style={[S.kpiVal, color ? {color} : {}]}>{value}</Text>
      <Text style={S.kpiLbl}>{label}</Text>
      {trend && <View style={[S.kpiTrendBadge, { backgroundColor: color ? color+'22' : C.primaryL }]}><Text style={[S.kpiTrend, { color: color || C.primary }]}>{trend}</Text></View>}
    </View>
  );
        ...
        <TouchableOpacity key={d.id} style={S.devCard} onPress={() => { connectDevice(d); setTab('devices'); }}>
          ...
          {d.bat != null && (
            <View style={{ height:4, backgroundColor:C.border, borderRadius:2, marginTop:6 }}>
              <View style={{ height:4, width:`${d.bat}%`, backgroundColor:d.bat>50?C.success:d.bat>20?C.warning:C.danger, borderRadius:2 }} />
            </View>
          )}
        ...
        <ScrollView style={S.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={S.filterRow}>
            ...
            {(['ASCII','HEX'] as const).map(m => (
              <TouchableOpacity key={m} style={[S.tabChip, consoleMode===m && S.tabChipA]} onPress={() => setConsoleMode(m)}>
    ...
    <ScrollView style={S.scroll} showsVerticalScrollIndicator={false}>
      ...
      <View style={S.chartCard}>
        <View style={S.chartArea}>
          ...
          {/* Sparkline bar row */}
          <View style={{ flexDirection:'row', alignItems:'flex-end', gap:4, marginTop:8, alignSelf:'stretch', justifyContent:'space-around', height:40, paddingHorizontal:8 }}>
            {[[-85,40],[ -78,25],[-68,50],[-55,35],[-72,40],[-65,55],[-80,30],[-70,45],[-62,50],[-75,38]].map(([v,h],idx) => (
              <View key={idx} style={{ flex:1, height:Math.max(8,h), backgroundColor:v<-75?C.warning:v<-65?C.primary:C.success, borderRadius:3, opacity:0.8 }} />
            ))}
          </View>
        </View>
      </View>
      ...
      <View style={S.chartCard}>
        <View style={S.chartArea}>
          ...
          {/* Sparkline bar row */}
          <View style={{ flexDirection:'row', alignItems:'flex-end', gap:4, marginTop:8, alignSelf:'stretch', justifyContent:'space-around', height:40, paddingHorizontal:8 }}>
            {[65,72,68,75,70,78,82,76,80,84].map((v,idx) => (
              <View key={idx} style={{ flex:1, height:Math.max(8,v*0.4), backgroundColor:v<30?C.danger:v<50?C.warning:C.success, borderRadius:3, opacity:0.8 }} />
            ))}
          </View>
        </View>
      </View>
  ...
  const MapTab = () => {
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        {/* Google Maps WebView */}
        <View style={{ flex: 1, margin: 8, borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: '#e5e7eb' }}>
          <WebView
            source={{ uri: mapUrl }}
            style={{ flex: 1 }}
  ...
  return (
    <SafeAreaView style={S.root}>
      ...
      <View style={S.header}>
        <View style={S.headerRow}>
          <View>
            <Text style={{ fontSize:20, fontWeight:'800', color:C.primary, letterSpacing:-0.5 }}>CloudTek</Text>
            <Text style={{ fontSize:11, color:C.muted, marginTop:1 }}>LoRaWAN Manager · v9.5</Text>
          </View>
      ...
      <View style={S.bottomNav}>
        {finalNav.map(item => (
          <TouchableOpacity key={item.key} style={S.navItem} onPress={() => { if(item.key !== 'devices') setSelectedDevice(null); if(item.key === 'map') setTab('map'); else setTab(item.key as any); }}>
            {tab === item.key && <View style={S.navActivePip} />}
